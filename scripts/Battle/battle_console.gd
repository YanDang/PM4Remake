...
func DealDamage(character: Node2D, goalenemy: Node2D, damage_type:int,offset:Vector2) -> void:
	var goalenemy_sprite = goalenemy.get_node("AnimatedSprite2D")
	var hit_target = DamageSettlement(character, goalenemy, damage_type)
	# 命中则播放受击动画
	if hit_target:
...
func DamageSettlement(character: Node2D, goalenemy:Node2D,damage_type:int) -> bool:
...
func AtkAction(character: Node2D, goalenemy:Node2D, offset:Vector2):
	action_list.hide()
	# 攻击方图层设置为1
	character.z_index = 1
	...
	tween.tween_callback(Callable(self, "DealDamage").bind(character, goalenemy, DamageType.PHYSICAL,offset))
	# 3. 等待攻击动画时间，然后回到原位
...
func _on_action_list_atk_pressed(character: Node2D, goalenemy: Node2D) -> void:
	...
	player_turn = true
	AtkAction(character, goalenemy, character_offset)
...
func _on_action_finished(character: Node2D, goalenemy: Node2D) -> void:
	if player_turn:
		...
		player_turn = false
		enemy_can_batter = true
		AtkAction(goalenemy, character, goalenemy_offset)
		CheckResult(character,goalenemy)
	else:
		if enemy_can_batter:
			var batter_rand = randi() % 100
			if batter_rand < goalenemy.batter:
				enemy_can_batter = false
				AtkAction(character,goalenemy,goalenemy_offset)
				return
		# 敌人连击结束，回到玩家回合
		player_turn = true
		CheckResult(character,goalenemy)
...
func CheckResult(character: Node2D, goalenemy: Node2D) -> void:
	var character_sprite = character.get_node("AnimatedSprite2D")
	var goalenemy_sprite = goalenemy.get_node("AnimatedSprite2D")
